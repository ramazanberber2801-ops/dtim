...
export default async function handler(req:VercelRequest,res:VercelResponse){
  ...
  if(!/^[0-9a-f-]{36}$/i.test(token))return res.status(400).send('Ugyldig billett');
  const supabase=createClient(supabaseUrl,serviceRoleKey,{auth:{persistSession:false,autoRefreshToken:false}});
  const {data,error}=await supabase.from('activity_registrations')
    .select('id,full_name,email,status,ticket_token,wallet_token,activity:organization_activities(id,title,activity_date,start_time,end_time,location,status,organization_id,organization:organizations(name,logo_url))')
