...
import { Bell, BellOff, ChevronRight, MapPin, Navigation, Phone, User } from 'lucide-react';
...
export function ContactPage(){
  ...
  useEffect(()=>{(async()=>{if(!('serviceWorker' in navigator))return;const registration=await navigator.serviceWorker.getRegistration('/sw.js');setNotificationsEnabled(Boolean(await registration?.pushManager.getSubscription()));})();},[]);

  const enableNotifications=async()=>{setNotificationLoading(true);setNotificationsEnabled(await subscribeToPushNotifications());setNotificationLoading(false);};
  ...
  return <div className="min-h-screen pb-28" dir={direction} style={{backgroundColor:brand.background,color:brand.text}}>
    ...
    <section className="relative z-10 -mt-8 px-4"><div className="overflow-hidden rounded-2xl border-2 shadow-lg" style={lightCardStyle}><div className="relative h-36 overflow-hidden"><img src="/images/map-banner.jpg" alt="" className="h-full w-full object-cover"/><div className="absolute inset-0" style={{background:`linear-gradient(to bottom, ${mix(brand.secondary,20)}, ${mix(brand.secondary,40)})`}}/><div className="absolute inset-0 flex items-center justify-center"><div className="theme-card flex h-14 w-14 items-center justify-center rounded-full border-2 shadow-lg" style={{borderColor:mix(brand.primary,30)}}><MapPin size={26} style={{color:brand.primary}} fill="currentColor"/></div></div></div><div className="p-5"><p className="mb-1 text-[10px] font-semibold uppercase tracking-wider" style={{color:brand.primary}}>{t('contact.location')}</p><h3 className="mb-2 font-serif text-lg">{t('contact.address')}</h3>{settings.address&&<p className="whitespace-pre-line text-sm leading-relaxed opacity-60">{settings.address}</p>}{settings.mapUrl&&<a href={settings.mapUrl} target="_blank" rel="noopener noreferrer" onClick={()=>void trackEvent('contact_click','map',t('contact.directions'))} className="theme-secondary-panel mt-4 flex w-full items-center justify-center gap-2 rounded-xl py-3.5 text-sm font-semibold shadow-md"><Navigation size={17}/>{t('contact.directions')}</a>}</div></div></section>
