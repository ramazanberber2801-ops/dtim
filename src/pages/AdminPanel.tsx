  ...
  return (
    <div className="p-4">
      <form onSubmit={submit} className="space-y-4">
        ...
        <div className="bg-white rounded-xl p-4 border-2 border-[#C5A880]/25 space-y-3">
          <h3 className="font-serif text-lg">Vipps / Bağış</h3>
          <input 
            className={inputClass} 
            value={form.vippsNumber || ''} 
            onChange={e => change('vippsNumber', e.target.value)} 
            placeholder="Vipps Numarası" 
          />
          ...
          {isSuperadmin && (
            <>
              ...
              <input
                className={inputClass}
                value={form.vippsDonationUrl || ''}
                onChange={e => change('vippsDonationUrl', e.target.value)}
                placeholder="https://qr.vipps.no/..."
              />
              <p className="text-xs text-[#2D2A26]/60">
                Lim inn Vipps Donation URL fra Vipps-portalen. Hvis feltet er tomt,
                brukes vanlig Vipps-nummer.
              </p>
            </>
          )}

          <label className="flex items-start gap-2 text-sm text-[#2D2A26]/80">
            <input type="checkbox" className="mt-1" checked={form.vippsButtonEnabled !== false} onChange={e => change('vippsButtonEnabled', e.target.checked)} />
            <span>
              <span className="font-medium">Vipps uygulamasında aç butonu aktif</span>
              <br />
              <span className="text-xs text-[#2D2A26]/50">Açık: Vipps butonu görünür. Kapalı: sadece numara ve kopyalama görünür.</span>
            </span>
          </label>
        </div>
        ...
        {isSuperadmin && (
          <div className="bg-white rounded-xl p-4 border-2 border-[#C5A880]/25 space-y-3">
            ...
            <label className="flex items-center gap-2 text-sm font-medium"><input type="checkbox" checked={form.ramadanEnabled || false} onChange={e => change('ramadanEnabled', e.target.checked)} />Ramazan Modu Aktif</label>
            <input type="date" className={inputClass} value={form.ramadanStartDate || ''} onChange={e => change('ramadanStartDate', e.target.value)} />
            <input type="date" className={inputClass} value={form.ramadanEndDate || ''} onChange={e => change('ramadanEndDate', e.target.value)} />
          </div>
        )}
        ...
        {isSuperadmin && (
          <div className="bg-white rounded-xl p-4 border-2 border-[#C5A880]/25 space-y-3">
            ...
            <label className="flex items-center gap-2 text-sm"><input type="checkbox" checked={!!form.kurbanEnabled} onChange={e => change('kurbanEnabled', e.target.checked)} />Kurban Bayramı Aktif</label>
            <input type="date" className={inputClass} value={form.kurbanStartDate || ''} onChange={e => { change('kurbanStartDate', e.target.value); change('kurbanEnabled', !!e.target.value); }} />
          </div>
        )}
        
        {saved && <p className="text-sm text-green-700 flex items-center gap-2"><Check size={16} /> Kaydedildi.</p>}
        <button type="submit" className="w-full py-3 rounded-lg bg-[#C5A880] text-white font-medium flex items-center justify-center gap-2"><Save size={16} /> Ayarları Kaydet</button>
      </form>
      ...
      <form onSubmit={changePassword} className="mt-6 bg-white rounded-xl p-4 border-2 border-[#C5A880]/25 space-y-3">
        ...
        <button type="submit" className="w-full py-3 rounded-lg bg-[#2D2A26] text-[#FAF6F0] font-medium">Şifreyi Güncelle</button>
      </form>
    </div>
  );
}
