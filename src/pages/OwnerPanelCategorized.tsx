...
const moduleCategories: ModuleCategory[] = [
  { id: 'core', title: '🏛️ Grunnmoduler', description: 'Basisfunksjoner for de fleste organisasjoner.', items: [
    ...
    { id: 'calendar', name: 'Kalender', type: 'Core', enabled: false },
    { id: 'contact', name: 'Kontakt', type: 'Core', enabled: true, locked: true },
    { id: 'about', name: 'Om oss', type: 'Core', enabled: false },
    { id: 'documents', name: 'Dokumenter', type: 'Core', enabled: false },
  ]},
  { id: 'islamic', title: '🕌 Islamske moduler', description: 'Funksjoner for moskeer og islamske organisasjoner.', items: [
    { id: 'prayer', name: 'Bønnetider', type: 'Islam', enabled: false },
    { id: 'jumuah', name: 'Fredagsbønn', type: 'Islam', enabled: false },
    { id: 'ramadan', name: 'Ramadan', type: 'Islam', enabled: false },
    { id: 'eid', name: 'Eid', type: 'Islam', enabled: false },
    { id: 'kurban', name: 'Kurban', type: 'Islam', enabled: false },
    { id: 'ayet', name: 'Ayet', type: 'Islam', enabled: false },
    { id: 'hadis', name: 'Hadis', type: 'Islam', enabled: false },
    { id: 'dua', name: 'Du’a', type: 'Islam', enabled: false },
    { id: 'qibla', name: 'Qibla', type: 'Islam', enabled: false },
    { id: 'islamic-calendar', name: 'Islamsk kalender', type: 'Islam', enabled: false },
  ]},
  { id: 'members', title: '👥 Medlemmer', description: 'Medlemsregister, roller og frivillige.', items: [
    { id: 'members', name: 'Medlemsregister', type: 'Medlem', enabled: false },
    { id: 'volunteers', name: 'Frivillige', type: 'Medlem', enabled: false },
    { id: 'board', name: 'Styret', type: 'Medlem', enabled: true },
    { id: 'roles', name: 'Roller', type: 'Medlem', enabled: false },
  ]},
  { id: 'finance', title: '💰 Økonomi', description: 'Donasjoner, kontingent og sponsor.', items: [
    { id: 'donation', name: 'Donasjon', type: 'Økonomi', enabled: true },
    { id: 'zakat', name: 'Zakat', type: 'Økonomi', enabled: false },
    { id: 'sadaqah', name: 'Sadaqah', type: 'Økonomi', enabled: false },
    { id: 'membership-fee', name: 'Medlemskontingent', type: 'Økonomi', enabled: false },
    { id: 'sponsor', name: 'Sponsor', type: 'Økonomi', enabled: false },
  ]},
  { id: 'education', title: '📚 Undervisning', description: 'Kurs, sohbet og undervisningsinnhold.', items: [
    { id: 'sohbet', name: 'Sohbet', type: 'Undervisning', enabled: true },
    { id: 'courses', name: 'Kurs', type: 'Undervisning', enabled: false },
    { id: 'quran-education', name: 'Koranundervisning', type: 'Undervisning', enabled: false },
    { id: 'kids-youth', name: 'Barn og ungdom', type: 'Undervisning', enabled: false },
    { id: 'video-archive', name: 'Videoarkiv', type: 'Undervisning', enabled: false },
  ]},
  { id: 'communication', title: '📢 Kommunikasjon', description: 'Varsler, meldinger og kontakt med brukere.', items: [
    { id: 'push', name: 'Push-varsler', type: 'Kommunikasjon', enabled: true },
    { id: 'chat', name: 'Chat', type: 'Kommunikasjon', enabled: false },
    { id: 'messages', name: 'Meldinger', type: 'Kommunikasjon', enabled: false },
    { id: 'email', name: 'E-post', type: 'Kommunikasjon', enabled: false },
  ]},
  { id: 'design', title: '🎨 Design', description: 'Branding, tema og layout.', items: [
    { id: 'branding', name: 'Branding', type: 'Design', enabled: true },
    { id: 'themes', name: 'Tema', type: 'Design', enabled: false },
    { id: 'layouts', name: 'Layout', type: 'Design', enabled: false },
    { id: 'splash', name: 'Splash', type: 'Design', enabled: false },
    { id: 'icons', name: 'Ikoner', type: 'Design', enabled: false },
  ]},
  { id: 'admin', title: '⚙️ Administrasjon', description: 'Admin, import, eksport og integrasjoner.', items: [
    { id: 'admins', name: 'Administratorer', type: 'Admin', enabled: true },
    { id: 'export', name: 'Eksport', type: 'Admin', enabled: false },
    { id: 'import', name: 'Import', type: 'Admin', enabled: false },
    { id: 'api', name: 'API', type: 'Admin', enabled: false },
    { id: 'integrations', name: 'Integrasjoner', type: 'Admin', enabled: false },
  ]},
  { id: 'analytics', title: '📊 Analyse', description: 'Statistikk, aktivitet og rapporter.', items: [
    { id: 'stats', name: 'Statistikk', type: 'Analyse', enabled: true },
    { id: 'reports', name: 'Rapporter', type: 'Analyse', enabled: false },
    { id: 'activity', name: 'Aktivitet', type: 'Analyse', enabled: false },
  ]},
];
...
const flatModules = moduleCategories.flatMap((cat) => cat.items);
...
export function OwnerPanel() {
  ...
  const [modules, setModules] = useState<ModuleItem[]>(flatModules);
  ...
  async function loadOrgs() { if (!supabase) return; const { data, error } = await supabase.from('organizations').select('*').order('name', { ascending: true }); if (error) { setDbStatus('Supabase-tabell mangler'); return; } const loaded = (data || []).map(fromDb); if (loaded.length > 0) { setOrgs(loaded); setSelectedId(loaded[0].id); setForm(loaded[0]); } setDbStatus('Koblet til Supabase'); }
  ...
  function toggleModule(id: string) { setModules((prev) => prev.map((mod) => mod.id === id && !mod.locked ? { ...mod, enabled: !mod.enabled } : mod)); }
