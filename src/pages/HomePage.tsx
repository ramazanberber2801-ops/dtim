...
export function HomePage() {
  ...
  const prayerEnabled = enabled('prayer', true);
  const ramadanEnabled = enabled('ramadan', true);
  const kurbanEnabled = enabled('kurban', true);
  ...
  return (
    <div className="min-h-screen pb-28" style={{ backgroundColor: brand.background, color: brand.text }}>
      ...
      {!modulesLoading && prayerEnabled && (
        <section className="px-4 pt-4">
          <div className="overflow-hidden rounded-xl border-2 shadow-md" style={cardStyle}>
            <div className="relative flex items-center justify-between px-4 py-2.5" style={{ backgroundColor: brand.secondary, color: brand.secondaryText }}>
              <div className="flex min-w-0 items-center gap-1.5">
                {isAuto ? <Crosshair size={13} style={{ color: brand.primary }} /> : <MapPin size={13} style={{ color: brand.primary }} />}
                <span className="max-w-[145px] truncate text-xs font-medium">{city.name}{city.country ? `, ${city.country}` : ''}</span>
              </div>
              <button onClick={() => setShowSelector(!showSelector)} className="text-[10px]" style={{ color: brand.primary }}>Şehir Değiştir</button>
              {showSelector && (
                <div className="absolute left-0 right-0 top-full z-20 mt-1 rounded-b-xl border-2 border-t-0 p-3 shadow-2xl" style={{ backgroundColor: brand.background, borderColor: mix(brand.primary, 28) }}>
                  <div className="relative">
                    <Search size={15} className="absolute left-3 top-1/2 -translate-y-1/2 opacity-30" />
                    <input value={searchQuery} onChange={(event) => handleSearch(event.target.value)} placeholder="Şehir ara..." className="w-full rounded-lg border bg-white py-2.5 pl-9 pr-9 text-sm" />
                    {searching && <Loader2 size={14} className="absolute right-3 top-1/2 -translate-y-1/2 animate-spin" />}
                  </div>
            ...
            {nextPrayer && (
              <div className="flex items-center justify-between border-b px-4 py-2" style={{ backgroundColor: mix(brand.primary, 9), borderColor: mix(brand.primary, 16) }}>
                <span className="flex items-center gap-1.5 text-[11px]"><Clock size={13} />Sıradaki: <strong>{nextPrayer.name}</strong></span>
                <span className="text-[11px] font-semibold">{nextPrayer.time} · {timeUntil}</span>
              </div>
            )}

            <div className="grid grid-cols-3 gap-1.5 p-2.5">
              {loading ? <p className="col-span-3 py-4 text-center text-xs opacity-50">Namaz vakitleri yükleniyor...</p> : prayerItems.map(({ name, time, icon: Icon }) => (
                <div key={name} className="rounded-lg border-2 p-2 text-center" style={{ backgroundColor: brand.background, borderColor: mix(brand.primary, 18) }}>
                  <Icon size={14} className="mx-auto mb-1" style={{ color: brand.primary }} />
                  <p className="text-[10px] opacity-60">{name}</p>
      ...
      {!modulesLoading && dailyInspirationEnabled && dailyData && (
        <section className="px-4 pt-4">
          <div className="rounded-xl p-5" style={darkCardStyle}>
            {dailyData.verse_text && <div><div className="mb-2 flex items-center gap-2"><BookOpen size={14} style={{ color: brand.primary }} /><h3 className="text-[11px] font-bold uppercase" style={{ color: brand.primary }}>Bugünün Ayeti</h3></div><p className="text-sm">{dailyData.verse_text}</p>{dailyData.verse_reference && <p className="mt-1 text-[10px] opacity-50">{dailyData.verse_reference}</p>}</div>}
            {dailyData.hadith_text && <div className={dailyData.verse_text ? 'mt-5' : ''}><div className="mb-2 flex items-center gap-2"><BookOpen size={14} style={{ color: brand.primary }} /><h3 className="text-[11px] font-bold uppercase" style={{ color: brand.primary }}>Bugünün Hadisi</h3></div><p className="text-sm">{dailyData.hadith_text}</p>{dailyData.hadith_source && <p className="mt-1 text-[10px] opacity-50">{dailyData.hadith_source}</p>}</div>}
          </div>
        </section>
      )}

      {!modulesLoading && activitiesEnabled && (
        <section className="px-4 pt-5">
          <div className="mb-2 flex items-center gap-2"><Mic size={18} style={{ color: brand.primary }} /><h2 className="font-serif text-lg">Yaklaşan aktiviteler</h2></div>
          <div className="space-y-2.5">
            {upcomingActivities.length === 0 ? <div className="rounded-xl border p-5 text-center text-sm opacity-50" style={cardStyle}>Yaklaşan aktivite yok.</div> : upcomingActivities.map((item) => (
              <button key={item.id} onClick={() => openActivity(item)} className="flex w-full overflow-hidden rounded-xl border text-left" style={cardStyle}>
                <div className="flex w-16 shrink-0 flex-col items-center justify-center py-3" style={{ backgroundColor: brand.secondary, color: brand.secondaryText }}><span className="text-[10px]" style={{ color: brand.primary }}>{new Date(item.date).toLocaleDateString('tr-TR', { month: 'short' })}</span><span className="font-serif text-xl">{new Date(item.date).getDate()}</span><span className="text-[10px] opacity-60">{item.time}</span></div>
                <div className="min-w-0 flex-1 p-3"><h3 className="truncate font-serif text-sm">{item.title}</h3><p className="mt-1 line-clamp-2 text-xs opacity-50">{item.description}</p><div className="mt-1.5 flex gap-3 text-[10px] opacity-40"><span className="flex items-center gap-1"><MapPin size={10} />{item.location}</span>{item.speaker && <span className="flex items-center gap-1"><User size={10} />{item.speaker}</span>}</div></div>
              </button>
            ))}
          </div>
        </section>
      ...
      {!modulesLoading && newsEnabled && (
        <section className="px-4 pt-5">
          <div className="mb-3 flex items-center gap-2"><Newspaper size={18} style={{ color: brand.primary }} /><h2 className="font-serif text-lg">Nyheter</h2></div>
          <div className="space-y-2.5">
            {featuredNews.length === 0 ? <div className="rounded-xl border p-6 text-center text-sm opacity-50" style={cardStyle}>Ingen nyheter ennå.</div> : featuredNews.map((item) => {
              const image = item.imageBase64 || item.image_base_64 || item.image_base64;
              return <button key={item.id} onClick={() => { void trackEvent('news_open', item.id, item.title); setSelectedNews(item); }} className="flex w-full rounded-xl border p-2 text-left" style={cardStyle}>{image ? <img src={image} alt="" className="h-20 w-20 shrink-0 rounded-lg object-cover" /> : <div className="flex h-20 w-20 shrink-0 items-center justify-center rounded-lg" style={{ backgroundColor: mix(brand.primary, 10) }}><Newspaper size={20} /></div>}<div className="min-w-0 p-2"><h3 className="line-clamp-2 font-serif text-sm">{item.title}</h3><p className="mt-1 line-clamp-2 text-xs opacity-50">{item.content}</p></div></button>;
            })}
          </div>
        </section>
      )}
