...
const sections = [
  ...
  { id: 'administration' as const, label: 'Styret', icon: ShieldCheck },
...
export function OrganizationAdminPortal() {
  ...
  return <div className="min-h-full" style={{ backgroundColor: brand.background, color: brand.text }}>
    ...
    <div className="mx-auto grid max-w-6xl gap-4 p-4 sm:p-6 lg:grid-cols-[220px_minmax(0,1fr)]">
      ...
      <main>{activeSection==='dashboard'?<section className="rounded-3xl border p-5 shadow-sm" style={{backgroundColor:brand.card,borderColor:mix(brand.primary,16)}}><h3 className="font-serif text-xl">Velkommen til {session.organizationName}</h3><p className="mt-2 text-sm opacity-65">Medlemmer, nyheter, aktiviteter, varslinger, styret og organisasjonsinnstillinger er koblet til denne organisasjonen.</p></section>:activeSection==='members'?<MembersModule organizationId={session.organizationId}/>:activeSection==='news'?<NewsModule organizationId={session.organizationId}/>:activeSection==='activities'?<ActivitiesModule organizationId={session.organizationId}/>:activeSection==='notifications'?<ManualPushModule/>:activeSection==='administration'?<OrganizationStaffModule organizationId={session.organizationId}/>:<OrganizationSettingsModule organizationId={session.organizationId}/>}</main>
    </div>
  </div>;
}
