...
const mapNews=(row:any)=>({id:row.id,title:row.title||'',content:row.content||row.summary||'',summary:row.summary||'',category:'Duyuru',date:row.published_at||row.created_at,image_base64:row.image_url||'',imageBase64:row.image_url||'',status:row.status,visibility:row.visibility||'public'});
...
export const AppProvider:React.FC<{children:React.ReactNode}>=({children})=>{
  ...
  const loadAllData=async()=>{
    ...
    try{
      ...
      if(n.data)setNews(n.data.map(mapNews)); else setNews([]);
    ...
    }catch(e){console.error('Kunne ikke laste organisasjonsdata:',e);}finally{setLoading(false);}
  };
  ...
  const addSohbet=async(item:any)=>{if(!supabase)return;const shouldSendPush=item._sendPush===true;const payload=activityPayload(item);const {error}=await supabase.from('organization_activities').insert(payload);if(error)return alert('Aktiviteten kunne ikke opprettes: '+error.message);if(shouldSendPush)await sendPush('Ny aktivitet',payload.title);await loadAllData();};
  const updateSohbet=async(id:string,item:any)=>{if(!supabase)return;const shouldSendPush=item._sendPush===true;const payload=activityPayload(item);const {error}=await supabase.from('organization_activities').update(payload).eq('id',id).eq('organization_id',organizationId);if(error)return alert('Aktiviteten kunne ikke oppdateres: '+error.message);if(shouldSendPush)await sendPush('Aktiviteten er oppdatert',payload.title);await loadAllData();};
  const deleteSohbet=async(id:string)=>{if(!supabase)return;const {error}=await supabase.from('organization_activities').delete().eq('id',id).eq('organization_id',organizationId);if(error)return alert('Aktiviteten kunne ikke slettes: '+error.message);await loadAllData();};
  const updateSettings=async(s:any)=>{if(!supabase)return;const {data,error}=await supabase.from('organization_settings').upsert(mapSettingsToDb(s,organizationId),{onConflict:'organization_id'}).select('*').single();if(error)return alert('Innstillingene kunne ikke lagres: '+error.message);setSettings(mapSettingsFromDb(data));};
  const updateInspiration=async(updates:any)=>{if(!supabase||!inspiration?.id)return;const {error}=await supabase.from('inspiration').update(updates).eq('id',inspiration.id);if(error)return alert('Teksten kunne ikke oppdateres: '+error.message);await loadAllData();};
  const addAdmin=async(admin:any)=>{if(!supabase)return;const cleanAdmin={id:admin.id,username:admin.username,display_name:admin.display_name||admin.displayName||admin.username,role:admin.role||'admin',auth_user_id:admin.auth_user_id||null};const {error}=await supabase.from('admins').insert([cleanAdmin]);if(error)return alert('Administratoren kunne ikke opprettes: '+error.message);await loadAllData();};
  const deleteAdmin=async(id:string)=>{if(!supabase)return;const {error}=await supabase.from('admins').delete().eq('id',id);if(error)return alert('Administratoren kunne ikke slettes: '+error.message);await loadAllData();};
  const updateAdminPassword=async(_id:string,newPass:string)=>{if(!supabase)return;const {error}=await supabase.auth.updateUser({password:newPass});if(error)alert('Passordet kunne ikke oppdateres: '+error.message);};
...
export const useApp=()=>{const context=useContext(AppContext);if(!context)throw new Error('useApp må brukes inne i AppProvider');return context;};
