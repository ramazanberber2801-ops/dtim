...
import { Activity, AlertCircle, BellRing, CalendarDays, Edit3, Loader2, MapPin, Plus, Search, Trash2, Users, X } from 'lucide-react';
...
const emptyForm: ActivityForm = { title: '', description: '', activityDate: '', startTime: '', endTime: '', location: '', capacity: '', status: 'draft' };
...
const activityPushBody = (item: Pick<ActivityItem, 'description' | 'activityDate' | 'startTime' | 'location'>) => {
  const details = [item.activityDate, item.startTime, item.location].filter(Boolean).join(' · ');
  return item.description.trim() || details || 'En ny aktivitet er publisert i appen.';
};
...
export function ActivitiesModule({ organizationId }: { organizationId: string }) {
  ...
  const loadItems = async () => {
    ...
    if (error) {
      setItems([]);
      setLoadError(error.message.includes('organization_activities') ? 'Aktivitetstabellen er ikke klar i Supabase ennå.' : error.message);
    } else {
  ...
  const filteredItems = useMemo(() => {
    ...
    return items.filter((item) => {
      ...
      return !needle || [item.title, item.description, item.location].join(' ').toLowerCase().includes(needle);
    });
  ...
  const sendItemPush = async (item: Pick<ActivityItem, 'title' | 'description' | 'activityDate' | 'startTime' | 'location'>) => {
    await sendPushNotification({ title: `Aktivitet: ${item.title}`, body: activityPushBody(item) });
  };
  ...
  const saveItem = async (event: FormEvent) => {
    ...
    try {
      if (editingItem) {
        const { error } = await supabase.from('organization_activities').update(payload).eq('id', editingItem.id).eq('organization_id', organizationId);
  ...
  const deleteItem = async (item: ActivityItem) => {
    if (!supabase || !window.confirm(`Slette aktiviteten «${item.title}»?`)) return;
    setActionId(item.id);
    const { error } = await supabase.from('organization_activities').delete().eq('id', item.id).eq('organization_id', organizationId);
    setActionId('');
    if (error) return alert('Aktiviteten kunne ikke slettes: ' + error.message);
    await loadItems();
  };

  const resendPush = async (item: ActivityItem) => {
    setActionId(item.id);
    try {
      await sendItemPush(item);
    ...
    <div className="space-y-4">
      <section className="rounded-3xl border p-5 shadow-sm" style={{ backgroundColor: brand.card, borderColor: mix(brand.primary, 16), color: brand.text }}>
        <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          <div><p className="text-xs uppercase tracking-[0.18em] opacity-45">Kjernemodul</p><h3 className="font-serif text-2xl">Aktiviteter</h3><p className="mt-1 text-sm opacity-60">{items.length} aktiviteter i organisasjonen.</p></div>
          <button type="button" onClick={openCreate} className="flex items-center justify-center gap-2 rounded-xl px-4 py-3 text-sm font-medium" style={{ backgroundColor: brand.primary, color: 'var(--brand-primary-text)' }}><Plus size={17} /> Ny aktivitet</button>
        </div>
        <div className="mt-5 grid gap-3 sm:grid-cols-[minmax(0,1fr)_180px]">
          <label className="flex items-center gap-2 rounded-xl border bg-white px-3" style={{ borderColor: mix(brand.primary, 18) }}><Search size={16} className="opacity-45" /><input className="w-full bg-transparent py-3 text-sm outline-none" value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Søk aktivitet eller sted" /></label>
          <select className="rounded-xl border bg-white px-3 py-3 text-sm" value={statusFilter} onChange={(e) => setStatusFilter(e.target.value as 'all' | ActivityStatus)}><option value="all">Alle statuser</option><option value="published">Publisert</option><option value="draft">Utkast</option><option value="cancelled">Avlyst</option></select>
        </div>
      </section>

      {loading ? <div className="flex items-center justify-center gap-2 rounded-2xl border bg-white p-8 text-sm opacity-60"><Loader2 size={18} className="animate-spin" /> Henter aktiviteter...</div>
      : loadError ? <div className="flex items-start gap-3 rounded-2xl border border-red-200 bg-red-50 p-4 text-red-800"><AlertCircle size={19} /><div><p className="text-sm font-semibold">Aktivitetsmodulen kan ikke laste data</p><p className="mt-1 text-xs">{loadError}</p></div></div>
      : filteredItems.length === 0 ? <div className="rounded-2xl border bg-white p-8 text-center"><Activity size={28} className="mx-auto opacity-30" /><p className="mt-3 text-sm font-medium">Ingen aktiviteter funnet</p></div>
      : <div className="space-y-2">{filteredItems.map((item) => (
        <article key={item.id} className="rounded-2xl border bg-white p-4 shadow-sm" style={{ borderColor: mix(brand.primary, 14), color: brand.text }}>
          <div className="flex items-start gap-3">
            <div className="flex h-12 w-12 shrink-0 flex-col items-center justify-center rounded-xl text-xs font-semibold" style={{ backgroundColor: mix(brand.primary, 10), color: brand.primary }}><CalendarDays size={17} /><span>{item.activityDate ? item.activityDate.slice(8, 10) : '—'}</span></div>
            <div className="min-w-0 flex-1"><div className="flex items-center gap-2"><p className="truncate text-sm font-semibold">{item.title}</p><span className={`rounded-full px-2 py-0.5 text-[10px] ${statusClass(item.status)}`}>{statusLabel(item.status)}</span></div><p className="mt-1 truncate text-xs opacity-50">{item.activityDate}{item.startTime ? ` · ${item.startTime}` : ''}{item.location ? ` · ${item.location}` : ''}</p>{(item.capacity || item.description) && <p className="mt-1 truncate text-[11px] opacity-40">{item.capacity ? `Kapasitet ${item.capacity}` : item.description}</p>}</div>
          </div>
      ...
      {editorOpen && <div className="fixed inset-0 z-[120] flex items-end justify-center bg-black/45 sm:items-center sm:p-4"><div className="max-h-[92vh] w-full max-w-xl overflow-y-auto rounded-t-3xl bg-white p-5 shadow-2xl sm:rounded-3xl sm:p-6" style={{ color: brand.text }}>
        <div className="flex items-center justify-between"><div><p className="text-xs uppercase tracking-[0.18em] opacity-45">Aktiviteter</p><h3 className="font-serif text-2xl">{editingItem ? 'Rediger aktivitet' : 'Ny aktivitet'}</h3></div><button type="button" onClick={closeEditor} className="flex h-10 w-10 items-center justify-center rounded-xl bg-black/5"><X size={18} /></button></div>
