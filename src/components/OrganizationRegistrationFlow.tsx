...
import { Building2, CheckCircle2, Loader2, ShieldCheck, Sparkles, Stethoscope } from 'lucide-react';
...
export function OrganizationRegistrationFlow(){
  ...
  const [form,setForm]=useState({organizationName:'',organizationType:initialType,country:'Norge',language:'Norsk',adminName:'',email:'',password:'',slug:''});
  ...
  return <div className="min-h-screen bg-slate-50 px-4 py-8 text-slate-900"><div className="mx-auto max-w-xl"><div className="mb-6 text-center"><div className={`mx-auto flex h-14 w-14 items-center justify-center rounded-2xl text-white ${isClinic?'bg-fuchsia-600':'bg-blue-600'}`}>{isClinic?<Stethoscope size={26}/>:<Building2 size={26}/>}</div><p className="mt-4 text-xs font-semibold uppercase tracking-[0.2em] text-slate-400">Yasaflow selvbetjening</p><h1 className="mt-2 text-3xl font-semibold">Opprett {isClinic?'klinikken':'organisasjonen'} din</h1><p className="mt-2 text-sm text-slate-500">7 dager gratis prøveperiode. Ingen betaling registreres nå.</p></div>
    <form onSubmit={submit} className="space-y-4 rounded-3xl bg-white p-6 shadow-xl">
      <div className="grid grid-cols-2 gap-2 rounded-2xl bg-slate-100 p-1"><button type="button" onClick={()=>setForm({...form,organizationType:'Klinikk'})} className={`rounded-xl px-3 py-3 text-sm font-semibold ${isClinic?'bg-white text-fuchsia-700 shadow-sm':'text-slate-500'}`}><span className="flex items-center justify-center gap-2"><Stethoscope size={16}/> Klinikk</span></button><button type="button" onClick={()=>setForm({...form,organizationType:'Forening'})} className={`rounded-xl px-3 py-3 text-sm font-semibold ${!isClinic?'bg-white text-blue-700 shadow-sm':'text-slate-500'}`}><span className="flex items-center justify-center gap-2"><Building2 size={16}/> Organisasjon</span></button></div>
      ...
      <div><label className="text-sm font-medium">Ditt navn</label><input required className="mt-1 w-full rounded-xl border p-3" placeholder={`Navn på ${ownerLabel}`} value={form.adminName} onChange={e=>setForm({...form,adminName:e.target.value})}/></div>
      ...
      <div className={`rounded-2xl p-4 text-sm ${isClinic?'bg-fuchsia-50 text-fuchsia-950':'bg-blue-50 text-blue-900'}`}><div className="flex gap-2"><ShieldCheck size={18} className="shrink-0"/><p>Yasaflow reserverer appadressen og oppretter {entityLabel}, eierkonto, standardtema, moduler og 7 dagers prøveperiode.</p></div>{isClinic&&<div className="mt-3 flex gap-2 border-t border-fuchsia-200 pt-3"><Sparkles size={18} className="shrink-0"/><p>Klinikken får sitt eget dashboard og kan selv administrere profil, ansatte, innhold, kunder og aktive klinikkmoduler.</p></div>}</div>
