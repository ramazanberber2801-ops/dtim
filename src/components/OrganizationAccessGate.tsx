...
export function OrganizationAccessGate({ children }: { children: ReactNode }) {
  ...
  useEffect(()=>{
    if(!supabase){setState('allowed');return;}
    let cancelled=false;

    const load=async()=>{
      const {data:org}=await supabase.from('organizations').select('app_access_mode').eq('id',DEFAULT_ORGANIZATION_ID).maybeSingle();
      const nextMode=(org?.app_access_mode||'public') as AccessMode;
      if(cancelled)return;
      setMode(nextMode);

      if(nextMode!=='authenticated'){setState('allowed');return;}

      const {data:{session}}=await supabase.auth.getSession();
      if(!session){setState('denied');return;}

      const [membershipResult,adminResult]=await Promise.all([
        supabase.from('organization_user_memberships').select('status').eq('organization_id',DEFAULT_ORGANIZATION_ID).eq('user_id',session.user.id).maybeSingle(),
        supabase.from('organization_admins').select('id').eq('organization_id',DEFAULT_ORGANIZATION_ID).eq('user_id',session.user.id).maybeSingle(),
      ]);
      if(cancelled)return;
      if(adminResult.data){setState('allowed');return;}
      if(membershipResult.data?.status==='active'){setState('allowed');return;}
      if(membershipResult.data?.status==='pending'){setState('pending');return;}
      setState('denied');
    };
    ...
    return()=>{cancelled=true;data.subscription.unsubscribe();window.removeEventListener('yasaflow-membership-changed',refresh);window.removeEventListener('yasaflow-organization-settings-changed',refresh);};
  },[]);

  if(mode!=='authenticated'||state==='allowed')return <>{children}</>;

  return <div className="flex min-h-screen items-center justify-center px-5" style={{backgroundColor:'var(--brand-background, #F4FAFF)',color:'var(--brand-text, #071B53)'}}>
