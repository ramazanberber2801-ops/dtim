...
export function ArrangementProTools({activity,organizationId}:{activity:Activity;organizationId:string}){
  ...
  const issueCertificate=async(row:Registration)=>{if(!supabase)return;setBusy(`certificate-${row.registration_id}`);setMessage('');const {data,error}=await supabase.rpc('issue_activity_certificate',{p_registration_id:row.registration_id});if(error)setMessage(error.message.includes('attendance_required')?'Deltakeren må være sjekket inn før kursbevis kan utstedes.':error.message);else{const token=(data as {certificate_token?:string})?.certificate_token||row.certificate_token;if(token)window.open(`/api/activity-certificate?token=${encodeURIComponent(token)}`,'_blank','noopener,noreferrer');await load();}setBusy('');};
  ...
  const createEvaluations=async()=>{if(!supabase)return;setBusy('evaluations');setMessage('');const {data,error}=await supabase.rpc('create_activity_evaluation_invites',{p_activity_id:activity.id});if(error)setMessage(error.message);else setMessage(`${Number(data||0)} evalueringsinvitasjon(er) ble opprettet.`);setBusy('');};
  ...
  return <section className="mt-5 rounded-2xl border p-4">
    ...
    <div className="mt-3 grid gap-2 sm:grid-cols-2">
      ...
      <button type="button" disabled={busy==='evaluations'} onClick={()=>void createEvaluations()} className="flex items-center justify-center gap-2 rounded-xl border py-3 text-xs font-semibold disabled:opacity-50">{busy==='evaluations'?<Loader2 size={15} className="animate-spin"/>:<MessageSquareText size={15}/>}Opprett evalueringer</button>
    </div>
