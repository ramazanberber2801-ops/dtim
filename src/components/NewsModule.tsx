...
export function NewsModule({organizationId,pushEnabled=false}:{organizationId:string;pushEnabled?:boolean}){
  ...
  const openCreate=()=>{setEditingItem(null);setForm(emptyForm);setSendPush(pushEnabled);setSaveError('');setEditorOpen(true);};
  ...
  const saveItem=async(event:FormEvent)=>{
    ...
    try{
      ...
      if(pushEnabled&&sendPush)await sendItemPush({title,summary:form.summary,content:form.content});
  ...
  const resendPush=async(item:NewsItem)=>{if(!pushEnabled)return;setActionId(item.id);try{await sendItemPush(item);alert(t('news.pushResent'));}catch(error){alert(error instanceof Error?error.message:t('news.pushFailed'));}finally{setActionId('');}};
  ...
  return <div className="space-y-4">
    <section className="rounded-3xl border p-5 shadow-sm" style={{backgroundColor:brand.card,borderColor:mix(brand.primary,16),color:brand.text}}>
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between"><div><p className="text-xs uppercase tracking-[0.18em] opacity-45">{t('news.coreModule')}</p><h3 className="font-serif text-2xl">{t('news.title')}</h3><p className="mt-1 text-sm opacity-60">{items.length} {t('news.countSuffix')}</p></div><button type="button" onClick={openCreate} className="flex items-center justify-center gap-2 rounded-xl px-4 py-3 text-sm font-medium" style={{backgroundColor:brand.primary,color:'var(--brand-primary-text)'}}><Plus size={17} aria-hidden="true"/>{t('news.new')}</button></div>
      <div className="mt-5 grid gap-3 sm:grid-cols-[minmax(0,1fr)_180px]"><label className="flex items-center gap-2 rounded-xl border bg-white px-3" style={{borderColor:mix(brand.primary,18)}}><Search size={16} className="opacity-45" aria-hidden="true"/><input aria-label={t('news.search')} className="w-full bg-transparent py-3 text-sm outline-none" value={query} onChange={(e)=>setQuery(e.target.value)} placeholder={t('news.search')}/></label><select aria-label={t('news.allStatuses')} className="rounded-xl border bg-white px-3 py-3 text-sm" value={statusFilter} onChange={(e)=>setStatusFilter(e.target.value as 'all'|NewsStatus)}><option value="all">{t('news.allStatuses')}</option><option value="published">{t('news.published')}</option><option value="draft">{t('news.draft')}</option></select></div>
    </section>

    {loading?<div role="status" aria-live="polite" className="flex items-center justify-center gap-2 rounded-2xl border bg-white p-8 text-sm opacity-60"><Loader2 size={18} className="animate-spin" aria-hidden="true"/>{t('news.loading')}</div>:loadError?<div role="alert" className="flex items-start gap-3 rounded-2xl border border-red-200 bg-red-50 p-4 text-red-800"><AlertCircle size={19} aria-hidden="true"/><div><p className="text-sm font-semibold">{t('news.loadFailedTitle')}</p><p className="mt-1 text-xs">{loadError}</p></div></div>:filteredItems.length===0?<div className="rounded-2xl border bg-white p-8 text-center"><Newspaper size={28} className="mx-auto opacity-30" aria-hidden="true"/><p className="mt-3 text-sm font-medium">{t('news.empty')}</p></div>:<div className="space-y-2">{filteredItems.map((item)=><article key={item.id} className="rounded-2xl border bg-white p-4 shadow-sm" style={{borderColor:mix(brand.primary,14),color:brand.text}}><div className="flex items-start gap-3"><div className="flex h-11 w-11 shrink-0 items-center justify-center overflow-hidden rounded-xl" style={{backgroundColor:mix(brand.primary,10),color:brand.primary}}>{item.imageUrl?<img src={item.imageUrl} alt="" className="h-full w-full object-cover"/>:<Newspaper size={19} aria-hidden="true"/>}</div><div className="min-w-0 flex-1"><div className="flex items-center gap-2"><p className="truncate text-sm font-semibold">{item.title}</p><span className={`rounded-full px-2 py-0.5 text-[10px] ${item.status==='published'?'bg-green-100 text-green-700':'bg-amber-100 text-amber-700'}`}>{item.status==='published'?t('news.published'):t('news.draft')}</span></div><p className="mt-1 line-clamp-2 text-xs opacity-50">{item.summary||item.content||t('news.noSummary')}</p></div></div><div className={`mt-3 grid gap-2 border-t pt-3 ${pushEnabled?'grid-cols-3':'grid-cols-2'}`} style={{borderColor:mix(brand.primary,12)}}><button type="button" onClick={()=>openEdit(item)} aria-label={`${t('news.edit')}: ${item.title}`} className="flex items-center justify-center gap-1 rounded-lg bg-black/5 px-2 py-2 text-xs"><Edit3 size={14} aria-hidden="true"/>{t('news.edit')}</button><button type="button" disabled={actionId===item.id} onClick={()=>void deleteItem(item)} aria-label={`${t('news.delete')}: ${item.title}`} className="flex items-center justify-center gap-1 rounded-lg bg-red-50 px-2 py-2 text-xs text-red-700 disabled:opacity-50"><Trash2 size={14} aria-hidden="true"/>{t('news.delete')}</button>{pushEnabled&&<button type="button" disabled={actionId===item.id} onClick={()=>void resendPush(item)} aria-label={`${t('news.pushAgain')}: ${item.title}`} className="flex items-center justify-center gap-1 rounded-lg px-2 py-2 text-xs disabled:opacity-50" style={{backgroundColor:mix(brand.primary,12),color:brand.primary}}><BellRing size={14} aria-hidden="true"/>{t('news.pushAgain')}</button>}</div></article>)}</div>}

    {editorOpen&&<div className="fixed inset-0 z-[120] flex items-end justify-center bg-black/45 sm:items-center sm:p-4"><div role="dialog" aria-modal="true" aria-labelledby="news-editor-title" className="max-h-[92vh] w-full max-w-xl overflow-y-auto rounded-t-3xl bg-white p-5 shadow-2xl sm:rounded-3xl sm:p-6" style={{color:brand.text}}><div className="flex items-center justify-between"><div><p className="text-xs uppercase tracking-[0.18em] opacity-45">{t('news.title')}</p><h3 id="news-editor-title" className="font-serif text-2xl">{editingItem?t('news.editTitle'):t('news.new')}</h3></div><button type="button" onClick={closeEditor} aria-label={t('news.close')} className="flex h-10 w-10 items-center justify-center rounded-xl bg-black/5"><X size={18} aria-hidden="true"/></button></div><form onSubmit={saveItem} className="mt-5 space-y-4"><div><label className="text-xs font-medium">{t('news.formTitle')} *</label><input required className="mt-1 w-full rounded-xl border px-3 py-3 text-sm" value={form.title} onChange={(e)=>setForm((p)=>({...p,title:e.target.value}))}/></div><div><label className="text-xs font-medium">{t('news.summary')}</label><input className="mt-1 w-full rounded-xl border px-3 py-3 text-sm" value={form.summary} onChange={(e)=>setForm((p)=>({...p,summary:e.target.value}))}/></div><div><label className="text-xs font-medium">{t('news.content')}</label><textarea rows={7} className="mt-1 w-full rounded-xl border px-3 py-3 text-sm" value={form.content} onChange={(e)=>setForm((p)=>({...p,content:e.target.value}))}/></div><div><label className="text-xs font-medium">{t('news.imageUrl')}</label><input type="url" className="mt-1 w-full rounded-xl border px-3 py-3 text-sm" value={form.imageUrl} onChange={(e)=>setForm((p)=>({...p,imageUrl:e.target.value}))}/></div><div><label className="text-xs font-medium">{t('news.status')}</label><select className="mt-1 w-full rounded-xl border px-3 py-3 text-sm" value={form.status} onChange={(e)=>setForm((p)=>({...p,status:e.target.value as NewsStatus}))}><option value="draft">{t('news.draft')}</option><option value="published">{t('news.published')}</option></select></div>{pushEnabled&&<label className="flex items-start gap-3 rounded-xl border bg-amber-50 p-3"><input type="checkbox" className="mt-0.5 h-4 w-4" checked={sendPush} onChange={(e)=>setSendPush(e.target.checked)}/><span><span className="block text-sm font-medium">{t('news.sendPush')}</span><span className="block text-xs opacity-60">{editingItem?t('news.pushEditHelp'):t('news.pushCreateHelp')}</span></span></label>}{saveError&&<p role="alert" className="rounded-xl bg-red-50 px-3 py-2 text-xs text-red-700">{saveError}</p>}<button type="submit" disabled={saving} className="flex w-full items-center justify-center gap-2 rounded-xl py-3 text-sm font-medium disabled:opacity-60" style={{backgroundColor:brand.primary,color:'var(--brand-primary-text)'}}>{saving&&<Loader2 size={16} className="animate-spin" aria-hidden="true"/>}{saving?t('news.saving'):editingItem?t('news.saveChanges'):t('news.create')}</button></form></div></div>}
  </div>;
}